...
mp_install_args="--accept yes --relax no"
...
if [ -n "$offline" ]; then
  ...
  mp_install_args="$mp_install_args --nodeps"
fi

echo
NUXEO_CONF=$NUXEO_HOME/bin/nuxeo.conf $NUXEO_HOME/bin/nuxeoctl mp-install $mp_install_args $packages
